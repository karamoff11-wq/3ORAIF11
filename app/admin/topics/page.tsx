...
const FALLBACK_CATS = [
  ...
  { id: 'cat-movie-dk',       topic_id: 'topic-movies',    name: 'The Dark Knight',      icon: '🦇' },
  ...
  { id: 'cat-movie-hp',       topic_id: 'topic-movies',    name: 'Harry Potter',         icon: '⚡' },
  ...
  { id: 'cat-tv-bb',          topic_id: 'topic-tvshows',   name: 'Breaking Bad',         icon: '⚗️' },
  ...
  { id: 'cat-tv-st',          topic_id: 'topic-tvshows',   name: 'Stranger Things',      icon: '🚲' },
  { id: 'cat-tv-peaky',       topic_id: 'topic-tvshows',   name: 'Peaky Blinders',       icon: '🧢' },
  { id: 'cat-tv-prison',      topic_id: 'topic-tvshows',   name: 'Prison Break',         icon: '🔒' },
  { id: 'cat-tv-heist',       topic_id: 'topic-tvshows',   name: 'Money Heist',          icon: '🎭' },
  ...
  // FIX: replaced ☠️ (contains hidden ZWJ \u200D — crashes SWC parser) with plain anchor emoji
  { id: 'cat-anime-op',       topic_id: 'topic-anime',     name: 'One Piece',            icon: '⚓' },
  { id: 'cat-anime-dn',       topic_id: 'topic-anime',     name: 'Death Note',           icon: '📓' },
  { id: 'cat-anime-hxh',      topic_id: 'topic-anime',     name: 'Hunter x Hunter',      icon: '🎣' },
  { id: 'cat-anime-dbz',      topic_id: 'topic-anime',     name: 'Dragon Ball Z',        icon: '🐉' },
  { id: 'cat-anime-ds',       topic_id: 'topic-anime',     name: 'Demon Slayer',         icon: '🗡️' },
  { id: 'cat-anime-jjk',      topic_id: 'topic-anime',     name: 'Jujutsu Kaisen',       icon: '✊' },
  { id: 'cat-anime-fma',      topic_id: 'topic-anime',     name: 'Fullmetal Alchemist',  icon: '🦾' },
  ...
  { id: 'cat-game-mc',        topic_id: 'topic-videogames', name: 'Minecraft',           icon: '🧱' },
  { id: 'cat-game-witcher',   topic_id: 'topic-videogames', name: 'The Witcher 3',       icon: '🐺' },
  { id: 'cat-game-rdr2',      topic_id: 'topic-videogames', name: 'Red Dead 2',          icon: '🤠' },
  ...
  { id: 'cat-game-lol',       topic_id: 'topic-videogames', name: 'League of Legends',   icon: '🏆' },
  { id: 'cat-game-zelda',     topic_id: 'topic-videogames', name: 'Zelda: BOTW',         icon: '🧝' },
  { id: 'cat-game-gow',       topic_id: 'topic-videogames', name: 'God of War',          icon: '🪓' },
]
...
export default function AdminTopicsPage() {
  ...
  const [form, setForm] = useState({
    ...
    order_index: 0,
    video_url:   '',
    bg_style:    'bg-gradient-to-br from-gray-900 to-black',
  })
  ...
  async function load() {
    ...
    try {
      ...
      let rawCats:   any[] = catsRes.data   ?? []

      // Use fallback if DB is empty
      if (rawTopics.length === 0) {
        rawTopics = FALLBACK_TOPICS
        rawCats   = FALLBACK_CATS
      }
  ...
  function openEdit(tp: Topic) {
    setForm({
      ...
      name:        tp.name        ?? '',
      icon:        tp.icon        ?? '📂',
      color:       tp.color       ?? '#8b5cf6',
      order_index: tp.order_index ?? 0,
      video_url:   tp.video_url   ?? '',
      bg_style:    tp.bg_style    ?? 'bg-gradient-to-br from-gray-900 to-black',
    })
    setCropConfig(tp.crop_config ?? null)
    setEditing(tp)
    setModal(true)
  }
  ...
  async function handleVideoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      toast.error('خطأ في الرفع: ' + err.message)
    } finally {
      setUploading(false)
    }
  }
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {Array(6).fill(0).map((_, i) => (
            <div key={i} className="h-28 card-glass animate-shimmer rounded-2xl" />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {topics.map((tp, i) => (
            ...
            >
              ...
              <div className="absolute top-0 bottom-0 right-0 w-1 z-10" style={{ backgroundColor: tp.color ?? '#8b5cf6' }} />

              <div
                className="text-4xl w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 mr-2 z-10 relative"
                style={{ background: `${tp.color ?? '#8b5cf6'}20` }}
              >
                {tp.icon}
              </div>

              <div className="flex-1 min-w-0 z-10 relative">
                <div className="font-bold truncate text-lg" style={{ color: 'var(--color-text-primary)' }}>
                  {tp.name}
                </div>
                <div className="flex flex-wrap gap-1 mt-2">
                  {tp.categories && tp.categories.length > 0 ? (
                    tp.categories.map((cat: any) => (
                      <span
                        key={cat.id}
              ...
              <div className="flex flex-col gap-1 z-10 relative">
                ...
                <button
                  ...
                  style={{ padding: '4px 10px', fontSize: '12px', background: 'rgba(239,68,68,0.1)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.2)' }}
                >
                  حذف
                </button>
              </div>
              ...
              {!editing && (
                <div>
                  ...
                  <input
                    ...
                    placeholder="مثال: topic-geography"
                  />
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  {BG_STYLES.map(bg => (
                    <button
                      ...
                      className={`text-xs p-3 rounded-xl border-2 transition-all text-white/80 relative overflow-hidden ${
                        form.bg_style === bg.value ? 'border-white font-bold' : 'border-white/10 hover:border-white/30'
                      }`}
                    >
                      {bg.label}
                    </button>
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  <input
                    ...
                    placeholder="رابط مباشر للفيديو (MP4)"
                  />
                  <div className="relative">
                    ...
                    />
                    {uploading && (
                      <div className="absolute right-4 top-2 text-xs text-cyan-400 animate-pulse font-bold">
                        جاري الرفع...
                      </div>
                    )}
                  </div>
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {ICONS.map(icon => (
                    <button
                      ...
                      style={{
                        background: form.icon === icon ? `${form.color}40` : 'var(--color-surface-3)',
          ...
          >
            ...
            >
              ...
              <p style={{ color: 'var(--color-text-secondary)' }}>
                سيتم حذف الموضوع وجميع فئاته وأسئلته المرتبطة به. هل أنت متأكد؟
              </p>
      ...
      <AnimatePresence>
        {cropEditorUrl && (
          <MediaCropEditor
            ...
            mediaType={
              cropEditorUrl.includes('.mp4') || cropEditorUrl.includes('.webm')
                ? 'video'
                : 'image'
            }
