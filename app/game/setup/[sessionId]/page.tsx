...
function playMascotSound(teamIndex: number, enabled: boolean) {
  ...
  try {
    const a = new Audio(`/sounds/mascot-${teamIndex + 1}.mp3`)
    a.volume = 0.65
    a.play().catch(() => {})
  } catch {}
}
...
function SpaceCanvas({ teams }: { teams: Team[] }) {
  ...
  const astRef     = useRef<AsteroidBody[]>([])
  ...
  useEffect(() => {
    ...
    const init = (w: number, h: number) => {
      ...
      // Shooting stars
      shotsRef.current = Array.from({ length: 4 }, (_, i) => ({
        x: w, y: (i * 22 + 7) / 100 * h,
        vx: -(8 + i * 2.5), vy: 0.8 + i * 0.4,
        len: 55 + i * 12, op: 0,
        life: 0, maxLife: 28,
        active: false, timer: i * 190 + 80,
      }))

      // Asteroids — 8 initial
      astRef.current = Array.from({ length: 8 }, (_, i) => spawnAsteroid(i, w, h, teams, nextIdRef))
      nextIdRef.current = 8
    }
    ...
    const loop = () => {
      ...
      teams.forEach((team, i) => {
        const cx = i % 2 === 0 ? w * 0.25 : w * 0.78
        const cy = i % 2 === 0 ? h * 0.3  : h * 0.72
        const pulse = 0.85 + 0.15 * Math.sin(timeRef.current * 0.4 + i * 2)
        const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, w * 0.5 * pulse)
        grad.addColorStop(0, `${team.color}22`)
        grad.addColorStop(0.5, `${team.color}0a`)
      ...
      starsRef.current.forEach(s => {
        const op = 0.08 + 0.82 * (0.5 + 0.5 * Math.sin(timeRef.current * s.sp + s.ph))
        ctx.beginPath()
      ...
      shotsRef.current.forEach(s => {
        if (!s.active) {
          s.timer--
          if (s.timer <= 0) { s.active = true; s.x = w + 10; s.life = 0 }
          return
        }
        s.x  += s.vx; s.y += s.vy; s.life++
        s.op = Math.min(1, s.life / 6) * (1 - Math.max(0, s.life - s.maxLife + 6) / 6)
        if (s.life >= s.maxLife || s.x < -s.len) {
          s.active = false; s.timer = 180 + Math.floor(Math.random() * 220)
        }
        if (s.op > 0.01) {
          ctx.save()
      ...
      // Physics step
      stepPhysics(astRef.current, w, h, teams, nextIdRef)

      // Draw asteroids
      astRef.current.forEach(ast => drawAsteroid(ctx, ast))
  ...
  return (
    <canvas ref={canvasRef}
      ...
      style={{ background: 'radial-gradient(ellipse at 40% 35%, #0f0c2e 0%, #07071A 55%, #000009 100%)' }}
    />
  )
}
...
// ─────────────────────────────────────────────
// SidebarTopicPreview — hover mini card
// ─────────────────────────────────────────────
function SidebarTopicPreview({
  topic, anchorY, visible,
}: {
  topic: Topic | null
  anchorY: number
  visible: boolean
  ...
  const selColor = topic.color ?? '#8B5CF6'
  return (
    <AnimatePresence>
      {visible && (
        <motion.div
          ...
          className="fixed z-[60] w-52 rounded-3xl overflow-hidden border border-white/10 shadow-2xl pointer-events-none"
          style={{ left: 276, top: Math.max(72, anchorY - 60), backdropFilter:'blur(20px)', background:'rgba(10,10,30,0.9)' }}>
          {/* Banner */}
          <div className="relative h-24 overflow-hidden">
            {topic.banner_url
              ? <img src={topic.banner_url} alt="" className="w-full h-full object-cover opacity-55" />
              : <div className="w-full h-full" style={{ background:`linear-gradient(135deg, ${selColor}30 0%, transparent 70%)` }} />
            }
            <div className="absolute inset-0 bg-gradient-to-b from-transparent to-[#0a0a1e]" />
            <div className="absolute bottom-3 start-3 flex items-center gap-2">
              <span className="text-2xl">{topic.icon ?? '📚'}</span>
              <span className="text-sm font-black leading-tight">{topic.name}</span>
            </div>
          </div>
          {/* Stats */}
          <div className="px-4 py-3 flex items-center justify-between">
            <span className="text-[10px] text-white/30 font-bold">{topic.categories.length} فئة</span>
            <div className="w-2 h-2 rounded-full" style={{ background: selColor }} />
          </div>
        </motion.div>
      )}
    </AnimatePresence>
...
function MatchupPreview({
  ...
  for (let i=0; i<teams.length-1; i+=2) if (teams[i+1]) pairs.push([teams[i],teams[i+1]])

  return (
    <div className="flex flex-col items-center justify-center w-full h-full gap-8 py-6">
      {pairs.map(([a,b], pi) => (
        <div key={pi} className="flex items-center gap-10 w-full max-w-md">
          <motion.div initial={{ x:-80, opacity:0 }} animate={{ x:0, opacity:1 }}
            transition={{ delay:pi*0.14, type:'spring', stiffness:200 }}
            className="flex-1 flex flex-col items-center gap-2">
            <motion.div animate={{ x:[0,9,0] }} transition={{ duration:1.7, repeat:Infinity, ease:'easeInOut' }}>
              <Mascot state="idle" size={80} color={a.color} />
            </motion.div>
            <span className="font-black text-sm">{a.name}</span>
            <div className="w-full h-1 rounded-full" style={{ background:a.color, boxShadow:`0 0 10px ${a.color}80` }} />
          </motion.div>

          <motion.div initial={{ scale:0 }} animate={{ scale:1 }}
            transition={{ delay:pi*0.14+0.18, type:'spring', stiffness:280 }}
            className="flex flex-col items-center flex-shrink-0">
            <motion.span animate={{ scale:[1,1.18,1] }} transition={{ duration:1.3, repeat:Infinity }}
              className="text-3xl text-white/20">⚔</motion.span>
            <span className="text-[9px] font-black uppercase tracking-[0.3em] text-white/15 mt-1">vs</span>
          </motion.div>
          ...
          <motion.div initial={{ x:80, opacity:0 }} animate={{ x:0, opacity:1 }}
            ...
            className="flex-1 flex flex-col items-center gap-2">
            <motion.div animate={{ x:[0,-9,0] }} transition={{ duration:1.7, repeat:Infinity, ease:'easeInOut' }}>
              <Mascot state="idle" size={80} color={b.color} />
            </motion.div>
            <span className="font-black text-sm">{b.name}</span>
            <div className="w-full h-1 rounded-full" style={{ background:b.color, boxShadow:`0 0 10px ${b.color}80` }} />
          </motion.div>
        </div>
      ))}
...
// GeneratingScreen
// ─────────────────────────────────────────────
function GeneratingScreen({ teams, accentColor }: { teams:Team[]; accentColor:string }) {
  const t = useTranslator()
  const cards = ['🎯','🧠','⚡','🏆','💡','🎮','🔥','✨']
  return (
...
export default function GameSetupPage() {
  ...
  const handleSelectAll = useCallback(()=>{
    ...
    setSelectedCategories(prev=>{
      ...
      if (alreadySel.length===allIds.length) return outside
  ...
  useEffect(()=>{
    const h=(e:KeyboardEvent)=>{
      ...
      if (e.key==='ArrowLeft'||e.key==='ArrowRight') {
        ...
        if (e.key==='ArrowLeft'  && cur>0)                         setActiveTopic(filteredTopics[cur-1])
        if (e.key==='ArrowRight' && cur<filteredTopics.length-1)   setActiveTopic(filteredTopics[cur+1])
      }
    ...
    try {
      await Promise.all([
        (supabase.from('sessions') as any).update({
          name:sessionName,
          punishment_mode: punishEnabled ? punishMode : null,
      ...
      toast.error(e?.message??'Failed to launch'); setIsStarting(false); setIsGenerating(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-[#07071A] text-white flex flex-col overflow-hidden"
      ...
      <AnimatePresence>{isGenerating&&<GeneratingScreen teams={teams} accentColor={activeColor}/>}</AnimatePresence>
      ...
      <SidebarTopicPreview topic={hoveredTopic} anchorY={previewY} visible={!!hoveredTopic} />
          ...
          <div className="hidden md:block relative">
            <input type="text" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)}
              ...
              className="bg-white/[0.05] border border-white/[0.08] rounded-2xl py-2 px-4 text-sm
                outline-none focus:bg-white/[0.08] transition-all placeholder:text-white/20 w-48"/>
            {searchQuery&&(
              <button onClick={()=>setSearchQuery('')}
                className="absolute end-3 top-1/2 -translate-y-1/2 text-white/25 hover:text-white/60 text-xs">✕</button>
            )}
          </div>
              ...
              return (
                <button key={topic.id}
                  ...
                  onMouseEnter={e=>{
                    ...
                    setPreviewY((e.target as HTMLElement).getBoundingClientRect().top)
                  }}
                  ...
                  style={{
                    background:  isActive?`${topic.color??accentColor}18`:'transparent',
                    borderLeft:  isActive?`2px solid ${topic.color??accentColor}`:'2px solid transparent',
                  }}>
                  ...
                  <AnimatePresence>
                    {activeTopic.banner_url?(
                      ...
                      <motion.div key={`bg2-${activeTopic.id}`} initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
                        ...
                        style={{ background:`linear-gradient(135deg, ${activeTopic.color??accentColor}1e 0%, transparent 60%)` }}/>
                    )}
                  </AnimatePresence>
                  ...
                  <div className="flex items-center justify-between mb-5">
                    ...
                    <button onClick={handleSelectAll}
                      ...
                      {activeTopic.categories.every(c=>selectedCategories.includes(c.id))
                        ?t('setup_unselect_all'):t('setup_select_all')}
                    </button>
                  </div>
                  {activeTopic.categories.length===0?(
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5">
                      {activeTopic.categories.map((cat,i)=>(
                        <motion.div key={cat.id} initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }}
                          transition={{ delay:i*0.033 }}>
                          <CategoryCard3D
