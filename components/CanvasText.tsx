...
export function CanvasText({
  ...
  colors = ["#ff6b6b", "#4ecdc4", "#45b7d1", "#96ceb4", "#ffeaa7", "#dfe6e9", "#EC4899", "#8B5CF6"],
  animationDuration = 5,
  lineWidth = 1.8,
  lineGap = 8,
  curveIntensity = 50,
  ...
  useEffect(() => {
    ...
    const ascent = metrics.actualBoundingBoxAscent || height * 0.8;
    const descent = metrics.actualBoundingBoxDescent || height * 0.2;
    const baselineY = (height + ascent - descent) / 2;

    const numLines = Math.floor(height / lineGap) + 15;
    ...
    const animate = (currentTime: number) => {
      ...
      // 2. Fill text shape with background
      ctx.globalCompositeOperation = "source-in";
      ctx.fillStyle = bgColor;
      ctx.fillRect(0, 0, width, height);

      // 3. Draw flowing colorful sine waves atop the text shape
      ctx.globalCompositeOperation = "source-atop";
      for (let i = 0; i < numLines; i++) {
        const y = (i - 5) * lineGap;

        const curve1 = Math.sin(phase + i * 0.1) * curveIntensity;
        const curve2 = Math.sin(phase + 0.5 + i * 0.1) * curveIntensity * 0.6;
